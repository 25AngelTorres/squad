...
class modelNorma extends modelModelo{
	//Nombre de tabla
    private $nombre_tabla = 'NORMA';
	//LLave primaria
    private $pk = 'id_norma';
	...
	*/
	...
	/**
		FUNCION PARA OBTENER Y ORDENAR REGISTRO
		
		@atrib	Atributos a obtener de la BD
		@orden	nuevo ordenamiento 
	**/
	public function get_atrib_toSelect(){
	...
	**/
	public function get_atrib_byID($id){
		if(!isset($id)){
			echo("error no hay id para get_atrib_byID_subproceso");
		}else{
			$query = 'select * from '.$this->nombre_tabla.' where '.$this->pk.' = "'.(int)$id.'"';
			$rs = $this->consulta_sql($query);
			return $rs->GetArray();
		}
	}
	...
	**/
	public function get_atrib_byWhere($where){
		//Define las columnas a obtener de la BD
	...
	**/
	public function update_subproceso($id, $data){
		...
		if(!isset($id)){
			$this->mensajeSubproceso['danger']='Error en id_subproceso. No declarada';
		}else{
		if((int)$id==0){
			$this->mensajeSubproceso['danger']='Error en id_subproceso. Valor 0';
		}else{
		if($id==''){
			$this->mensajeSubproceso['danger']='Error en id_subproceso. Valor ""';
		}else{
			...
			if ( !$er->valida_texto2($data['name'],3,50 )){
				...
				$rows = $rs->GetArray();
				if(count($rows) > 0){
					$this->mensajeSubproceso['danger'] = 'Process NAME not available.'; 
				}else{
					// 'description' sin caracteres especiales y longitud de 0-300 caracteres
					if(!$er->valida_texto2($data['description'],0,300 )){
						$this->mensajeSubproceso['danger']='DESCRIPTION not valid, special characters are no allowed. length 0-300 characters.';
					}else{
							/**ACTUALIZA**/
							...
							if(empty($sql_update)){
								$this->mensajeSubproceso['warning']='Without changes.';
							}else{
	...
	**/
	public function insert_subproceso($data){
		...
		// 'name' sin caracteres especiales y longitud de 3-50 caracteres
		if ( !$er->valida_texto2($data['name'],3,50 )){
			$this->mensajeSubproceso['danger']='NAME not valid, special characters are no allowed. length 3-50 characters.';
		}else{
		// 'name' no existe en bd
		$rs = $this->consulta_sql('select * from '.$this->nombre_tabla.' where name = "'.$data['name'].'"');
		$rows = $rs->GetArray();
		if(count($rows) > 0){
			$this->mensajeSubproceso['danger'] = 'Subprocess NAME not available.'; 
		}else{
			// 'description' sin caracteres especiales y longitud de 0-300 caracteres
			if(!$er->valida_texto2($data['description'],0,300 )){
				$this->mensajeSubproceso['danger']='DESCRIPTION not valid, special characters are no allowed. length 0-300 characters.';
				...
				if(!(int)$data['id_proceso']>0){
					$this->mensajeSubproceso['danger']='ID_PROCESO not valid.';
				}else{
					/**INSERTA**/
